...
const TableContent = (props) => {
    ...
    const dataSource = countriesData.map(countryData => { 
        ...
        if (countryData.name === 'Åland') {
            return ({...countryData, name: 'Aland', key: uniqid(), continent: countryData.continent.name })
        } 
    ...
    const continents = Object.values(continentsData).map(continent => ({text: continent.name, value: continent.name}));
    ...
    const columns = [
        ...
        },
        {
            ...
            width: '5%',
            responsive: ['sm'],
            render: (text) => <span role="img" aria-label="flag">{text}</span>
        },
        {
            title: 'name',
            dataIndex: 'name',
